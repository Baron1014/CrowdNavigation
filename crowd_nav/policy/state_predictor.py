...
class StatePredictor(nn.Module):
    ...
    def compute_next_state(self, robot_state, action):
        ...
        if self.kinematics == 'holonomic':
            ...
            next_state[0] = next_state[0] + np.cos(next_state[6]) * action.v * self.time_step
            next_state[1] = next_state[1] + np.sin(next_state[6]) * action.v * self.time_step
            next_state[2] = np.cos(next_state[6]) * action.v
            next_state[3] = np.sin(next_state[6]) * action.v
